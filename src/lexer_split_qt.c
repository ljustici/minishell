...
/*   Updated: 2023/11/13 16:10:28 by ljustici         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (lex.s[*i] == '\'' && (is_spnltab(lex.s[*i - 1]) || *i == 0))
	{
		f_letter_pos = *i;
		span_until_quote(lex.s, i, lex.s[*i]);
        if (should_split(lex.s[*i]) == 0)
        {
            while(lex.s[*i] && should_split(lex.s[*i]) == 0)
                (*i)++;
        }
		lex.tokens[*j] = ft_substr(lex.s, f_letter_pos, (*i - f_letter_pos) + 1);
		printf("[j: %i --> %s]\n", *j, lex.tokens[*j]);
		if(!lex.tokens[*j])
...
/**
 * Spans all the characters that belong to a variable. Only letters, numbers
 * and underscores are allowed. Also $$ and $? and all the numbered variables
 * ($0, $1, etc).
*/
...
/**
 * Checks if a character is a double quote and then obtains the position of the closing one.
 * Spans until this position, assigning string tokens and variable tokens, if there are any.
 * Also spans the characters found after the closing double quote if they are not separators
 * and adds them to the last token.
*/
